...
def test_ebookmeta_valid(test_rootlib) -> None:
    test_fb2 = "262001.fb2"
    meta = ebookmeta.get_metadata(os.path.join(test_rootlib, test_fb2))
    assert meta is not None
    assert meta.title == "The Sanctuary Sparrow"
    assert len(meta.cover_image_data) == 56360
    assert meta.cover_media_type == "image/jpeg"


def test_ebookmeta_invalid(test_rootlib) -> None:
    test_bad_fb2 = "badfile2.fb2"
    meta = ebookmeta.get_metadata(os.path.join(test_rootlib, test_bad_fb2))
    assert meta is not None
    assert meta.title == ""
    assert meta.cover_image_data is None
